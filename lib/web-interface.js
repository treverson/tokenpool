...
module.exports =  {
  ...
  async init( web3, accountConfig, poolConfig , redisInterface ,testmode  )
  {
    ...
    {
       payments = await this.redisInterface.getRecentElementsOfListInRedis(('balance_payments:'+minerAddress.toString()),limitAmount)
       ...
       for( i in payments )
       {
         ...
         var transferDataJSON = await this.redisInterface.findHashInRedis('balance_transfer:'+minerAddress.toString(),paymentId);
    ...
    async getMinerBalanceTransfers(minerAddress,limitAmount)
    {
      ...
      var transfer_keys = await this.redisInterface.getResultsOfKeyInRedis('balance_transfer:'+minerAddress.toString())

      for(i in transfer_keys)
      {
        var transferId = transfer_keys[i];
        var transferDataJSON = await this.redisInterface.findHashInRedis('balance_transfer:'+minerAddress.toString(),transferId);
        var transferData = JSON.parse(transferDataJSON)
