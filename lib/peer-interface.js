 ...
 //var redis = require("redis");
  ...
  const UPDATE_VAR_DIFF_PERIOD = 5*60 * 1000; //30 seconds
...
//       console.log('invalid share digest')
  ...
  //    console.log("Update Vardiff")
      ...
      // TMP      var newVarDiff = await this.getUpdatedVarDiffForMiner(minerData,minerAddress)


             minerData.hashRate = await this.estimateMinerHashrate(minerAddress )

    // TMP        minerData.varDiff = newVarDiff;
    // TMP        minerData.validSubmittedSolutionsCount = 0;  //reset
...
//          var response =  await self.handlePeerShareSubmit(shareData.nonce,shareData.minerEthAddress,shareData.challengeNumber,shareData.digest,shareData.difficulty );
            if(shareData.difficulty !=8192){         //added mikers fixed diff
               console.error(shareData.minerEthAddress, shareData.difficulty);
            }
// MRS   var response =  await self.handlePeerShareSubmit(shareData.nonce,shareData.minerEthAddress,shareData.challengeNumber,shareData.digest,shareData.difficulty );
            var response =  await self.handlePeerShareSubmit(shareData.nonce,shareData.minerEthAddress,shareData.challengeNumber,shareData.digest,8192);
         }catch(err)
         {
...
//            console.log('got receipt',liveTransactionReceipt )
  ...
  //            console.log('transaction was mined and failed',txHash)
            ...
            //this one is for profile rendering     - RA 0429-
...
//     console.log('got miner total hashRate in KHs', hashrateSum)
  ...
  //
  //         PROCESS_DIFFICULTY = 8192;  // added mikers fixed diff
...
//TMP
             ...
             {
               minerData.varDiff  = 8192; // was minDiff
             }
