...
//          var response =  await self.handlePeerShareSubmit(shareData.nonce,shareData.minerEthAddress,shareData.challengeNumber,shareData.digest,shareData.difficulty );
            if(shareData.difficulty !=6144){         //added mikers fixed diff
               console.error(shareData.minerEthAddress, shareData.difficulty);
            }
            var response =  await self.handlePeerShareSubmit(shareData.nonce,shareData.minerEthAddress,shareData.challengeNumber,shareData.digest,6144);
         }catch(err)
         {
  ...
  async initJSONRPCServer(port)
     {
       ...
       if(port == 8586){
           ...
           PROCESS_DIFFICULTY = 6144;  // added mikers fixed diff
