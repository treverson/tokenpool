...
// RA -   var MAX_VARDIFF = 500000;
      var MAX_VARDIFF = 16384;
//
      if( minerVarDiff > MAX_VARDIFF ){
           minerVarDiff = MAX_VARDIFF;
      }
// RA - Add hardcoded DIFF
      if(minerAddress.toString() == '0xCC675e6688255169eFfeC1b556730ba2479a9EA3'){
             minerVarDiff = 16384;
      } else if (minerAddress.toString() == '0x066A61b16b1eB498b15dabd803DDF8B8592A88ee'){
          minerVarDiff = 8192;
      } else if (minerAddress.toString() == '0xB04915709CFB68da5cB82F23e8b6d8Fdaa21725c'){
          minerVarDiff = 8192;
      } else if (minerAddress.toString() == '0xE42e4b61797F148E6DA859b83DE7064D33F4012a'){
          minerVarDiff = 32768;
      } else if (minerAddress.toString() == '0xAF1b929594BA515A699A25aBc29DA00F29723da0'){
          minerVarDiff = 8192;
      } else if (minerAddress.toString() == '0x368d669ebF1b65F239cB25A9F6843b4BaD6d4e8e'){
          minerVarDiff = 4096;
      } else if (minerAddress.toString() == '0xa7389bdae0be73c10f5fdd429d2ba5258e5cd3ec'){
          minerVarDiff = 4096;
      } else if (minerAddress.toString() == '0x626d97c4f4300e9fbda1b5be7fda8a989438f1b1'){
         minerVarDiff = 8192;
      }
   return minerVarDiff;
...
// RA - END
   ...
   async monitorMinedPayments()
   {
    ...
    try {
      ...
      */
         ...
         var payment_txes = await this.redisInterface.getResultsOfKeyInRedis('unconfirmed_broadcasted_payment')
   ...
   async checkMinedPayments(payment_txes)
   {
     ...
     var REBROADCAST_WAIT_BLOCKS = 25;
     ...
     for(i in payment_txes)
     {
      ...
      var txDataJSON = await this.redisInterface.findHashInRedis('unconfirmed_broadcasted_payment',paymentId);
      var transactionData = JSON.parse(txDataJSON)

        console.log('check mined payment',paymentId, transactionData )
    //  var paymentId = transactionData.balancePaymentId;

      if( transactionData.confirmed == false && (transactionData.last_broadcast_block < (currentEthBlock - REBROADCAST_WAIT_BLOCKS)) )
      {

        var balanceTransferJSON = await this.redisInterface.findHashInRedis('balance_transfer',paymentId);
        var balanceTransfer = JSON.parse(txDataJSON)

        if(balanceTransfer.txHash == null)
        {
          //need to rebroadcast for sure
          await this.queueReplacementPayment(transactionData)

        }else if(balanceTransfer.confirmed == false)
        {
          //try to get receipt via txhash

          var txHash = balanceTransfer.txHash;
          ...
          if(liveTransactionReceipt != null )
          {
            ...
            transactionData.confirmed = true;//also called 'mined'
            ...
            //save as confirmed
            await this.redisInterface.storeRedisHashData('broadcasted_payment' ,paymentId,JSON.stringify(transactionData) )
   ...
   async queueReplacementPayment(paymentData)
   {
     ...
     {
        await this.redisInterface.storeRedisHashData('queued_replacement_payment' ,balancePaymentId ,JSON.stringify(paymentData) )
        console.log('queue replacement payment');


        //create a new queued transfer
         if(this.pool_env == "staging")
         {
           await this.tokenInterface.queueTokenTransfer(paymentData.ethMinerAddress, paymentData.amount, paymentData.balancePaymentId)
         }
