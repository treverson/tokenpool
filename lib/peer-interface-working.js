...
// Ravid - old diff->  const UPDATE_VAR_DIFF_PERIOD = 60 * 1000; //30 seconds
  const UPDATE_VAR_DIFF_PERIOD = 40 * 1000; //30 seconds
...
module.exports =  {
 ...
 // ravid 20180321- fix diff     if(minerData.validSubmittedSolutionsCount > 0 && avgFindingTime!= null  ){


          if( avgFindingTime < expectedFindingTime  )
          {
              minerVarDiff = Math.ceil(minerVarDiff * 1.2 ); //harder
          }else if( avgFindingTime > expectedFindingTime  )
          {
              minerVarDiff = Math.ceil(minerVarDiff / 1.2 ); //easier
          }
//}
// end 20180321 - fix diff  

// Ravid - 20180321- Add hard coded miner diff 
	console.log('minerAddress', minerAddress.toString());

        if(minerVarDiff < 1) minerVarDiff = 1;
		minerVarDiff = Math.min(minerVarDiff, 1024*64);
	if(minerAddress.toString() == '0x3fb278'){
	       minerVarDiff = 32*1024; 
	} else if(minerAddress.toString() == '0x329115')
	       	minerVarDiff = 32*1024; 
	} else if(minerAddress.toString() == '0xCC675e')
	  	minerVarDiff = 32*1024;  
	} else if(minerAddress.toString() == '0xE42e4b')
	      	minerVarDiff = 32*1024;   
	}
// Ravid - End fix diff - hard coded
